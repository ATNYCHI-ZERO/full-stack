...
This entanglement creates an oracle that leaks the Hamming weight of `m'`, byte by byte, through a clean timing channel. This is not a simple timing attack, nor a simple fault attack. It is a hybrid that is significantly more powerful and harder to mitigate than its components.
...
5.  This process is repeated for each byte of `m'`, leading to full message recovery. With a full message oracle, the attacker can then mount a key recovery attack using standard lattice techniques.

#### **4. Experimental Validation**

We validated the EFI attack on a standard embedded target: a Microchip ATSAM4L microcontroller with an ARM Cortex-M4 core, running a reference implementation of Kyber from the widely used **`pqm4` library**.
...
#### **7. References**
 ...
 Alkim, E., Ducas, L., Pöppelmann, T., & Schwabe, P. (2018). CRYSTALS-Kyber: a CCA-secure module-lattice-based KEM. In *2018 IEEE European Symposium on Security and Privacy (EuroS&P)*.

 Krämer, J., Pessl, P., & Schwabe, P. (2017). PQM4: a post-quantum crypto library for the ARM Cortex-M4. In *Proceedings of the 2017 ACM SIGSAC Conference on Computer and Communications Security (CCS)*.

 Barenghi, A., Breveglieri, L., Koren, I., & Naccache, D. (Eds.). (2012). *Fault analysis in cryptography*. Springer Science & Business Media.

 Primas, R., Pessl, P., & Mangard, S. (2017). Single-trace side-channel attacks on masked lattice-based encryption. In *International Conference on Smart Card Research and Advanced Applications (CARDIS)*.

 Ravi, P., Järpe, M., & Verbauwhede, I. (2021). KyberSlash: A new timing attack on the NIST PQC candidate Kyber. In *30th USENIX Security Symposium (USENIX Security 21)*.

 Barenghi, A., Bertoni, G., Parrinello, E., & Pelosi, G. (2010). Low voltage fault attacks on the AES. In *Workshop on Cryptographic Hardware and Embedded Systems (CHES)*.
