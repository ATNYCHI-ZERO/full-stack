...
This module exposes helpers that reproduce the exploratory notebook code
which combined a harmonic oscillator model ("K-Math" terminology) with
stochastic quantum fluctuations.  The goal is to make the experiment
reproducible from the command line while keeping the core simulation code
free from optional third-party imports until the results have been
produced.
...
written to disk so automated tools (or humans) can quickly inspect
numerical values without rerunning the simulation.
"""
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--duration", type=float, default=1.0, help="Simulation duration in seconds"
    )
    parser.add_argument(
        "--runs", type=int, default=200, help="Number of ensemble runs to average"
    )
